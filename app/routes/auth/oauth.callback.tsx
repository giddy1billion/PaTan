...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const fallbackRoute = getSafeAuthRoute(
    session.get("oauth:fallbackRoute") as string | undefined,
  );
  const storedState = session.get("oauth:state") as string | undefined;
  const storedProvider = parseOAuthProvider(
    session.get("oauth:provider") as string | undefined,
  );
  const redirectTo = getSafeRedirectTarget(
    session.get("oauth:redirectTo") as string | undefined,
  );
  ...
  if (providerError) {
    const mapped =
      providerError === "access_denied" ? "oauth-cancelled" : "oauth-denied";
    await logAuthSecurityEvent({
      request,
  ...
  try {
    ...
    await logAuthSecurityEvent({
      ...
      email: user.email,
      route: "/oauth/callback",
      metadata: { provider: storedProvider },
    });
    session.unset("oauth:state");
    session.unset("oauth:provider");
    session.unset("oauth:redirectTo");
