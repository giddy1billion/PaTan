...
import { consumePasswordResetToken, validatePasswordResetToken } from '~/utils/password-reset.server';
...
export const meta: MetaFunction = () => {
  return [
    ...
    { name: 'description', content: 'Set a new password for your PaTan account.' },
  ];
};
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return {
    token,
    emailHint: maskEmail(validation.email),
    challengeRequired,
  };
}
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const hasValidCsrf = await verifyCsrfToken({ request, submittedToken: csrfToken });
  if (!hasValidCsrf) {
    ...
    return redirect(`/reset-password?token=${encodeURIComponent(token)}&error=invalid-csrf`);
  }
  ...
  if (!rateLimit.allowed) {
    ...
    return redirect(`/reset-password?token=${encodeURIComponent(token)}&error=rate-limited`, {
  ...
  if (challengeRequired) {
    const challengeResult = await verifyBotDefenseToken({ token: botDefenseToken, request });
    if (!challengeResult.ok) {
      ...
      const errorCode = challengeResult.reason === 'provider-unconfigured' ? 'captcha-unavailable' : 'captcha-failed';
      return redirect(
  ...
  if (password !== confirmPassword) {
    return redirect(`/reset-password?token=${encodeURIComponent(token)}&error=reset-password-mismatch`, {
      headers: rateLimit.headers,
    });
  }
  ...
  if (!securePassword.valid) {
    const errorCode = securePassword.breachedCount > 0 ? 'breached-password' : 'weak-password';
    return redirect(`/reset-password?token=${encodeURIComponent(token)}&error=${errorCode}`, {
      headers: rateLimit.headers,
    });
  }
...
export default function ResetPasswordRoute() {
  const { token, emailHint, challengeRequired } = useLoaderData<typeof loader>();
  ...
  const shouldRenderBotChallenge =
    ...
    (challengeRequired || searchParams.get('challenge') === 'required' || errorCode === 'captcha-required' || errorCode === 'captcha-failed');
  ...
  return (
    <div className="min-h-screen page-modern flex flex-col">
      <header className="p-4">
        ...
        >
          <img src="/brand/logos/logo-sm.png" alt="" className="h-8 w-auto" aria-hidden="true" />
      ...
      <main id="main-content" className="page-modern flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-lg">
          <div className="page-hero-modern p-6 sm:p-8">
            <h1 className="font-heading text-2xl font-bold text-midnight text-center">Set a New Password</h1>
            <p className="mt-2 text-center text-[#64748B]">Account: {emailHint}</p>
            ...
            <Form method="post" className="form-modern mt-8 space-y-6">
              ...
              <input type="hidden" name="botDefenseToken" data-bot-token-input value="" />

              <div>
                <label htmlFor="password" className="block text-sm font-medium text-night">New password</label>
                <input
                  id="password"
                  name="password"
                  type="password"
              ...
              <div>
                <label htmlFor="confirmPassword" className="block text-sm font-medium text-night">Confirm password</label>
                <input
                  id="confirmPassword"
              ...
              {shouldRenderBotChallenge ? (
                <div className="space-y-2" aria-live="polite">
                  <p className="text-sm text-night/70">Complete the verification challenge to continue.</p>
                  {rootData?.botDefense?.provider === 'turnstile' ? (
                    <div className="cf-turnstile" data-sitekey={rootData.botDefense.siteKey} data-callback="onAuthBotDefenseToken" />
                  ) : (
                    <div className="g-recaptcha" data-sitekey={rootData?.botDefense?.siteKey} data-callback="onAuthBotDefenseToken" />
                  )}
                  <script
