...
export const meta: MetaFunction = () => {
  return [
    ...
    { name: 'description', content: 'Complete multi-factor verification to finish signing in.' },
  ];
};
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  const userId = session.get('preAuth:userId') as string | undefined;
  const email = session.get('preAuth:email') as string | undefined;
  const redirectTo = (session.get('preAuth:redirectTo') as string | undefined) ?? '/dashboard';
  const remember = session.get('preAuth:remember') === '1';
  const sessionChallengeId = session.get('preAuth:challengeId') as string | undefined;
  ...
  if (!hasValidCsrf) {
    ...
    return redirect(`/auth/mfa?challenge=${encodeURIComponent(sessionChallengeId)}&error=invalid-csrf`);
  }
  ...
  if (!rateLimit.allowed) {
    ...
    return redirect(`/auth/mfa?challenge=${encodeURIComponent(sessionChallengeId)}&error=rate-limited`, {
      headers: rateLimit.headers,
    });
  }

  if (intent === 'resend') {
    const riskScore = Number(session.get('preAuth:riskScore') as string | undefined) || 0;
    const riskReasonsValue = String(session.get('preAuth:riskReasons') ?? '');
    const riskReasons = riskReasonsValue ? riskReasonsValue.split(',').filter(Boolean) : [];
    ...
    return redirect(`/auth/mfa?challenge=${encodeURIComponent(challenge.challengeId)}&error=mfa-code-resent`, {
  ...
  if (!verified.ok) {
    ...
    const errorCode =
      verified.code === 'challenge-expired' || verified.code === 'too-many-attempts'
        ? 'mfa-expired'
        : 'mfa-invalid';

    return redirect(`/auth/mfa?challenge=${encodeURIComponent(sessionChallengeId)}&error=${errorCode}`, {
      headers: rateLimit.headers,
    });
  }
      ...
      <main id="main-content" className="page-modern flex-1 flex items-center justify-center p-4 sm:p-6">
        <div className="w-full max-w-lg">
          <div className="page-hero-modern p-6 sm:p-8">
            <h1 className="font-heading text-2xl font-bold text-midnight text-center">Verify Your Sign-In</h1>
            <p className="mt-2 text-center text-[#64748B]">We sent a verification code to {emailHint}</p>
            ...
            <Form method="post" className="form-modern mt-8 space-y-6">
              ...
              <div>
                <label htmlFor="code" className="block text-sm font-medium text-night">Verification code</label>
                <input
                  id="code"
                  name="code"
                  type="text"
