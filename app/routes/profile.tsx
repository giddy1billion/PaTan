...
export const meta: MetaFunction = () => {
  return [
    { title: "Your Profile | PaTan" },
    {
      name: "description",
      content:
        "Review your profile, story journey, and community footprint on PaTan.",
    },
  ];
};
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const [
    ...
    db.aspiration.count({
      where: { authorId: sessionUser.id, deletedAt: null },
    }),
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  await db.user.update({
    where: { id: current.id },
    data: { mfaEnabled: requestedMfaEnabled },
  });
  await logAuthSecurityEvent({
    request,
    eventType: requestedMfaEnabled
          ...
          <div className="mt-6 grid lg:grid-cols-3 gap-4 sm:gap-5">
            ...
            <section className="rounded-2xl border border-midnight/10 bg-white p-5 shadow-sm lg:col-span-2">
              ...
              <p className="mt-3 text-sm text-night/80 leading-relaxed">
                {" "}
                {profile.bio?.trim() ||
                  "Add a short bio to help others connect with your journey and purpose."}{" "}
              </p>{" "}
            ...
            <aside className="rounded-2xl border border-midnight/10 bg-white p-5 shadow-sm lg:col-span-3">
              ...
              <p className="mt-2 text-sm text-night/70">
                {" "}
                Protect your account with multi-factor verification for
              ...
              >
                ...
                <input
                  type="hidden"
                  name="mfaEnabled"
                  value={profile.mfaEnabled ? "false" : "true"}
                />{" "}
