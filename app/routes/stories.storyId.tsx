...
type WorkspaceData = {
  ...
  activeAspirationsCount: number;
  savedStoriesCount: number;
  authoredStoriesCount: number;
  ...
  recentPrivateReflections: Array<{
    id: string;
    title: string | null;
    content: string;
    updatedAt: Date;
...
export const meta: MetaFunction<typeof loader> = ({ data }) => {
  ...
  return [
    { title: `${data.story.title} | PaTan` },
    {
      ...
      content:
        data.story.excerpt ||
        `Read ${data.story.author.displayName}'s story on PaTan.`,
    },
  ];
};
...
export async function loader({ request, params }: LoaderFunctionArgs) {
  ...
  const story = await db.story.findFirst({
    where: {
      ...
      ...(user
        ? { AND: { OR: [{ privacy: 'PUBLIC' }, { authorId: user.id }] } }
        : { privacy: 'PUBLIC' }),
    },
    select: {
      id: true,
  ...
  const reflections = await db.comment.findMany({
    where: {
      storyId: story.id,
      deletedAt: null,
      parentId: null,
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  if (!user) {
    const redirectPath = storyParam ? `/stories/${storyParam}` : '/discover';
    return new Response(null, {
      status: 302,
      headers: {
        Location: `/login?redirectTo=${encodeURIComponent(redirectPath)}`,
      },
    });
  }
  ...
  const submittedStoryId = String(formData.get('storyId') ?? '').trim();
  ...
  const story = await db.story.findFirst({
    where: {
      ...
      OR: [{ id: storyParam }, { slug: storyParam }],
      AND: {
        OR: [{ privacy: 'PUBLIC' }, { authorId: user.id }],
      },
    },
    select: {
      id: true,
  ...
  if (story.id !== submittedStoryId) {
    return {
      error: 'Story mismatch detected. Please refresh and try again.',
      values: { content },
    } satisfies ActionData;
  }
            ...
            <section className="mt-10" aria-labelledby="reflections-heading">
              ...
              {isAuthenticated ? (
                <div className="mt-4 rounded-2xl border border-midnight/10 bg-white p-4 sm:p-5">
                  ...
                  <Form method="post" className="mt-3 space-y-3">
                    ...
                    <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
                      ...
                      >
                        {isSavingPrivate ? 'Saving private reflection...' : 'Save private reflection'}
                      </button>
                      <button
                        type="submit"
                        name="intent"
                        ...
                        aria-busy={isSharing}
                      >
                        {isSharing ? 'Sharing reflection...' : 'Share reflection'}
                      </button>
                    </div>
                  </Form>
                </div>
                ...
                <div className="mt-4 rounded-2xl border border-midnight/10 bg-white p-5 sm:p-6">
                  ...
                  <Link
                    to={`/login?redirectTo=${encodeURIComponent(`/stories/${story.id}`)}`}
                    className="mt-4 btn-primary inline-flex min-h-[44px] items-center"
                  >
                    Sign in to reflect
                  </Link>
                </div>
