...
import { completeOnboardingWithInterests, getOnboardingProfile } from "~/utils/users.server";
...
export const meta: MetaFunction = () => {
  return [
    ...
    {
      name: "description",
      content: "Choose your interests so PaTan can personalize your first dashboard and discovery experience.",
    },
  ];
};
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return {
    redirectTo,
    selectedInterests: profile.personalInterests,
  };
}
  ...
  return (
    <div className="min-h-screen page-modern flex flex-col">
      <header className="p-4 sm:p-6">
        ...
        >
          <img src="/brand/logos/logo-sm.png" alt="" aria-hidden="true" className="h-8 w-auto" />
      ...
      <main id="main-content" className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <section className="w-full max-w-3xl rounded-3xl border border-midnight/10 bg-white/95 p-6 sm:p-8 shadow-[0_15px_45px_rgba(13,43,69,0.08)]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-forest">Step 2 of 2</p>
          ...
          <p className="mt-3 text-sm sm:text-base text-night/70 leading-relaxed">
            Pick the topics you want to see first in your dashboard and discovery feed.
          </p>
                ...
                {INTEREST_OPTIONS.map((interest) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      />
                      <span className="text-sm font-medium text-night group-hover:text-midnight">{interest}</span>
                    </label>
                  );
                })}
              </div>
