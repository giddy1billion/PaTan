...
export const meta: MetaFunction = () => {
  return [
    { title: "Auth Security Audit | PaTan" },
    {
      name: "description",
      content:
        "Operational visibility into authentication security events and anomalies.",
    },
  ];
};
...
export default function SecurityAuthAuditRoute() {
  ...
  return (
    <main
      id="main-content"
      className="page-modern min-h-screen px-4 py-10 sm:px-6 lg:px-8"
    >
      {" "}
      <div className="mx-auto max-w-7xl">
        ...
        <header className="mb-8 flex flex-wrap items-center justify-between gap-4">
          ...
          <div>
            ...
            <p className="mt-2 text-night/70">
              Live security metrics across login, signup, OAuth, MFA, and reset
              workflows.
            </p>{" "}
          </div>{" "}
          <Link
        ...
        <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <article className="rounded-xl border border-mist bg-white p-4 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-bold text-midnight">
              {dashboard.totals.last24hRateLimited}
            </p>{" "}
          </article>{" "}
        </section>{" "}
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  ...
                  {dashboard.topFailureIps.length === 0 ? (
                    <tr>
                      {" "}
                      <td colSpan={2} className="py-3 text-night/60">
                        No recent failed-attempt hotspots.
                      </td>{" "}
                    </tr>
                  ) : (
                    dashboard.topFailureIps.map((entry) => (
                      <tr
                        key={`${entry.ipAddress}-${entry.failures}`}
                        className="border-b border-mist/60"
                      >
                        {" "}
                        <td className="py-2 pr-4 text-night">
                          {entry.ipAddress}
                        </td>{" "}
            ...
            <div className="mt-4 space-y-3">
              ...
              {dashboard.recentHighRiskEvents.length === 0 ? (
                <p className="text-sm text-night/60">
                  No high-risk events detected recently.
                </p>
              ) : (
                dashboard.recentHighRiskEvents.map((event) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-night/70">
                      {" "}
                      IP: {event.ipAddress ?? "unknown"} | Risk:{" "}
                      {event.riskScore ?? 0}{" "}
                    </p>{" "}
