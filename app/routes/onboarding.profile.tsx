...
import { getOnboardingProfile, updateOnboardingProfileStep } from "~/utils/users.server";
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  throw redirect(`/onboarding/interests?redirectTo=${encodeURIComponent(redirectTo)}`);
}
    ...
    <div className="min-h-screen page-modern flex flex-col">
      ...
      <main id="main-content" className="flex-1 flex items-center justify-center p-4 sm:p-6">
        <section className="w-full max-w-2xl rounded-3xl border border-midnight/10 bg-white/95 p-6 sm:p-8 shadow-[0_15px_45px_rgba(13,43,69,0.08)]">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-forest">Step 1 of 2</p>
          ...
          <Form method="post" className="mt-6 space-y-5">
            ...
            <div>
              <label htmlFor="displayName" className="block text-sm font-medium text-night">
                Display name
              </label>
              <input
                id="displayName"
            ...
            <div>
              <label htmlFor="username" className="block text-sm font-medium text-night">
                Username
              </label>
              <input
                id="username"
            ...
            <div>
              <label htmlFor="bio" className="block text-sm font-medium text-night">
                Short bio (optional)
              </label>
              <textarea
                id="bio"
